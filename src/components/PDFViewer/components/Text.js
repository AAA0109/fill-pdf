...
export default function Text({ width, height, updateHandler, deleteHandler, payload, file, x, y, pageScale }) {
  ...
  const init = async () => {
    ...
    updateHandler({width: width * scale, height: height * scale})
    if (!["image/jpeg", "image/png"].includes(file.type)) {
      canvas.current.toBlob(blob => {
        updateHandler({file: blob})
      });
    }
  }
  ...
  return (
    <Box position='absolute' left={0} top={0} style={{ userSelect: 'none', width: `${width + dw}px`, height: `${height + dh}px`, transform: `translate(${x + dx}px, ${y + dy}px)` }}>
      ...
      >
        <Box
          data-direction="left"
          className="resize-border h-full w-1 border-l cursor-ew-resize"
          left={0}
          top={0}
        />
        <Box
          data-direction="top"
          className="resize-border w-full h-1 border-t cursor-ns-resize"
          left={0}
          top={0}
        />
        <Box
          data-direction="bottom"
          className="resize-border w-full h-1 border-b cursor-ns-resize"
          left={0}
          bottom={0}
        />
        <Box
          data-direction="right"
          className="resize-border h-full w-1 border-r cursor-ew-resize"
          right={0}
          top={0}
        />
        <Box
